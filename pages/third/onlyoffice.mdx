...
```sh
docker save -o /home/onlyoffice.tar onlyoffice/documentserver:latest
```
将 `tar` 归档文件拷贝到内网服务器
scp …
...
1. 直接运行镜像（此处用第 2 步）
```sh
docker run -i -t -d -p 8081:80 --name onlyoffice --restart=always -e JWT_SECRET=false onlyoffice/documentserver
```

如果您希望单独安装 `ONLYOFFICE` 文档服务器，请使用此命令。

> 注：从 7.2 版本开始，我们添加了随机生成的 JWT 秘密。如果 JWT secret 没有在环境变量中指定，它会导致在每次 VM 或物理服务器重启时重新生成它，因此集成可能会出现问题。为避免此问题，请在 中指定您自己的值-e JWT_SECRET=my_jwt_secret。如果您不打算在安装中使用 JWT，请使用-e JWT_ENABLED=false.
...
```sh
docker run -itd -p 8081:80  -e JWT_ENABLED=false --name onlyoffice --restart=always \
...
server {
    ...
    location / {
        ...
        proxy_pass http://10.2.112.21:31992;
        proxy_set_header X-Forwarded-Host $the_host;
        proxy_set_header X-Forwarded-Proto $the_scheme;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
    }
...
建议采用审友封装好的镜像: [hub.yonyouaud.com/a8/documentserver](hub.yonyouaud.com/a8/documentserver)


在浏览器输入：`http://服务器地址:8081`